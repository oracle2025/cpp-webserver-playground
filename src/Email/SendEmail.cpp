...
SendEmail::SendEmail(string server, string user, string password)
    : SMTP_SERVER(std::move(server))
    , SMTP_USER(std::move(user))
    , SMTP_PASSWORD(std::move(password))
{
}
void SendEmail::sendHTML(
    const string& from,
    ...
    const string& body)
{
    ...
    try {
        ...
        message.addRecipient(
            MailRecipient(MailRecipient::PRIMARY_RECIPIENT, receiver));
...
void SendEmail::send(const Poco::Net::MailMessage& message) const
{
