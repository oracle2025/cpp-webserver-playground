...
int main()
{
    ...
    const auto SMTP_PASSWORD_2 = read_string_from_file(CONFIG_DIR "/smtp/password");
    ...
    try {
        ...
        auto pSecure = new SecureSMTPClientSession(SMTP_SERVER);
        pSecure->login();
        if (!pSecure->startTLS(pContext)) {
            throw std::runtime_error("Failed to start TLS connection.");
        }
        /*
         * Generate valid message-id to avoid error:
         * Messages missing a valid Message-ID header are not 550-5.7.1 accepted
         */

        pSecure->login(SMTPClientSession::AUTH_LOGIN, SMTP_USER, SMTP_PASSWORD);
        MailMessage message;
        message.addRecipient(MailRecipient(
            MailRecipient::PRIMARY_RECIPIENT, "richard.spindler@gmail.com"));
        ...
        std::string html
            = "<html><body>Hello, world!<br><b>Hello Bold</b></body></html>";
        const auto htmlInline = R"(<!doctype html>
<html>
  <head>
