...
std::vector<Todo> Todo::list() {
    ...
    select << "SELECT m_id, description, created_at, updated_at, checked FROM Todo",
            into(todo.m_id),
            into(todo.description),
            into(todo.created_at),
            into(todo.updated_at),
            into(todo.checked),
